...
const getStrandsGlobePoint = (index: number, total: number, time: number) => {
  ...
  const lat =
    (Math.floor(gridIdx / lonLines) / (latLines - 1)) * Math.PI - Math.PI / 2;
  ...
  const lonSector = Math.floor(
    ((((lon % (Math.PI * 2)) + Math.PI * 2) % (Math.PI * 2)) / (Math.PI * 2)) *
      8,
  );
  return {
    x: x * perspective,
...
export const drawStrandsParticle = (
  ...
  if (!strandsParticleState || !strandsParticleState.initialised) {
    const state = {
      ...
      pg: new Float32Array(COUNT),
      pb: new Float32Array(COUNT),
  ...
  for (let i = 0; i < COUNT; i++) {
    ...
    switch (phase) {
      ...
      case 3: {
        ...
        const ci = Math.min(
          STRANDS_GRADIENT.length - 1,
          Math.floor(norm * STRANDS_GRADIENT.length),
        );
 ...
 */
export const drawStrandsWatermark = (
  ctx: CanvasRenderingContext2D,
  w: number,
  h: number,
) => {
...
const detectBeat = (
  ...
  if (
    delta > state.beatThreshold &&
    now - state.cooldownRef.current > state.cooldownMs
  ) {
...
export const renderSlideshow = (
  ...
  if (currentImg) {
    drawSlideshowImage(
      ...
      w,
      h,
      state.fitRef.current,
      state.opacityRef.current,
    );
  }
};
