...
**Status:** **HISTORICAL + CONTEXT-BOUNDED ADDENDUM** (v4 implementation guidance is deprecated)  
**Revision note:** v4.1 keeps the historical migration contracts, but adds the 2026-05-22 rule for bringing ongoing S3 Studio / Gener8 web changes into S3-derived Everywear applets without violating the context-protocol module budget.

> **This document is no longer the live implementation source of truth.**
> Core migration decisions have been implemented in code and are now documented in:
>
> - **[WIKI.md](./WIKI.md)** : file-by-file developer reference (maintained)
> - **[CONTEXT.md](./CONTEXT.md)** : current OODA state, build status, safe next steps
> - **[ARCHITECTURE_MODULES_2026-05-21.md](./ARCHITECTURE_MODULES_2026-05-21.md)** : active context-bounded modularisation plan
>
> Use this file for migration history and contract rationale. For new S3 Studio
> web / Gener8 changes, follow the context-bounded addendum below before copying
> or adapting code into S3-derived applets.

## Active Addendum 2026-05-22: Context-Bounded Migration Rule

Every S3 Studio or Gener8 web change entering an S3-derived Everywear applet
must pass through the `context-protocol.skill` budget:

- no editable source file over the effective 16k-token code ceiling;
- no direct copy of S3 Studio monolith files into Everywear applets;
- shared code used by two or more applets moves to `packages/`;
- applet-only UI remains under the applet, split by view, hook, service, and component;
- runtime-heavy assets move to `~/.everywear/` only when the shell must broker lifecycle;
- each migrated module gets a `docs/wiki/...` Module Contract entry before or with the code move.

The practical meaning: "bring S3 Studio into Everywear" means bringing S3
product behaviour into the S3 applets, not making S3 Studio web the functional
source for Everywear OS itself. Everywear's base desktop may borrow S3 Studio's
desktop language, icon idioms, and shell ergonomics, but the platform shell
keeps its own architecture, routing, lifecycle, auth, and hardware ownership.

For S3-derived applets, preserve behaviour while translating the surface into
context-sized Everywear modules.

### Modularisation Gate Before Further Applet Migration

The current Everywear repo already contains migration-touch files over the hard
16k-token code ceiling. Further S3 applet migration is blocked until these
surfaces are split or hoisted:

| Surface | Current role | Gate action |
|---|---|---|
| `applets/gener8/web/src/components/VideoGeneratorModal.tsx` | Gener8 video tool | Split and hoist with Vid/S3 into `packages/video-modal/`. |
| `applets/vid/web/src/components/VideoGeneratorModal.tsx` | Vid video tool | Consume `packages/video-modal/`; remove duplicate local modal. |
| `applets/gener8/web/src/workers/videoRenderWorker.ts` + Vid copy | Shared render worker | Move to `packages/video-modal/src/workers/`; S3, Gener8, and Vid copies are byte-identical as of 2026-05-22. |
| `platform/everywear-os/src-tauri/src/lib.rs` | Shell command surface | Split into `state.rs`, `crash.rs`, and `commands/*` before adding more shell migration commands. |
| `applets/gener8/src-tauri/src/shim.rs` | Gener8 local HTTP API | Split by route group before adding more S3 shim endpoints. |

This is a targeted gate, not a whole-repo freeze. Applets and crates below the
hard ceiling can continue to evolve, but any migration step that touches an
oversized surface must split that surface first.

Current upstream web files that are known migration hazards:

| Source file | Observed size | Migration rule |
|---|---:|---|
| `s3studio-web/src/components/VideoGeneratorModal.tsx` | ~68.6k tokens | Split first, then hoist to `packages/video-modal/`; apply S3 deltas once for Gener8 + Vid. |
| `s3studio-web/src/components/CreatePanel.tsx` | ~54.7k tokens | Do not copy whole. Split into create view, prompt form, model/tier controls, API adapter, and job state hooks. |
| `s3studio-web/src/components/studio/StemStudio.tsx` | ~26.1k tokens | Split into stem import, separation job state, waveform/track UI, export controls. |
| `s3studio-web/src/services/api.ts` | ~19.8k tokens | Split by backend concern; Everywear calls local shim / shell transport, not direct Supabase-first S3 APIs. |
| `s3studio-web/src/App.tsx` | ~17.6k tokens | Use as behaviour reference only; Everywear app shell already owns routing and chrome. |

`VideoGeneratorModal.tsx` has small but real drift between S3, Gener8, and Vid.
The worker file is byte-identical across all three surfaces as of this pass, so
the worker is an immediate shared-package candidate.
...
| 9 | Web migration boundary | S3 Studio web remains the upstream behaviour source for S3-derived applets, while Everywear OS owns the platform shell boundaries. Oversized S3 files are split before landing; shared pieces go to workspace packages. |

---

## Document Conventions

- **Gener8 source** = `C:\Users\MAG MSI\Project Ace\S3 STUDIO\s-gener8\` (standalone S3 Gener8 Tauri app)
- **S3 web source** = `C:\Users\MAG MSI\Project Ace\S3 STUDIO\s3studio-web\` (standalone web app / behaviour source)
...
- Context budget follows `context-protocol.skill`: module working unit <= 65k tokens, source code slot <= 16k tokens
...
| **S3 Studio Web Surface** | Source UI Reference | NO | N/A | N/A | Functional source for S3-derived applets only. Everywear desktop borrows visual language where useful, but not applet behaviour or routing. Oversized React files are not copied as-is; they are translated into applet views and shared packages. |
...
**Decision rule for web code:** A S3 Studio web file may only land in an
S3-derived applet unchanged when it is below the 16k-token code ceiling and has
one owner. If it is larger, split first. If it is used by Gener8 and Vid, hoist
it to `packages/`. If it contains online/Supabase assumptions, port the
interface to the local Everywear shim or shell transport before merging. For the
Everywear OS shell, use S3 Studio web as visual/interaction reference only.
...
### S3 Studio Web (Source for S3 Applets)

S3 Studio web is the behaviour source for the current Gener8 / Studio applet
family, not for Everywear OS as a whole. The platform desktop borrows from S3's
base desktop language where appropriate, but S3 Studio web does not define
Everywear's shell lifecycle, app routing, hardware model, or entitlement
ownership.

Its large-file boundaries do not fit the Everywear agent-maintenance protocol.
Treat the following as S3 applet migration queues, not copy targets:

| Source surface | Classification | Everywear destination | Context-sensitive action |
|---|---|---|---|
| `components/VideoGeneratorModal.tsx` | Shared video creation surface | `packages/video-modal/` consumed by Gener8 + Vid | Split into `types`, `presets`, hooks, panels, preview, render controls; then apply upstream S3 delta once. |
| `workers/videoRenderWorker.ts` | Shared render worker | `packages/video-modal/src/workers/` | Hoist immediately; current S3, Gener8, and Vid copies are byte-identical. |
| `lib/silhouetteEngine.ts` | Shared visualizer engine | `packages/visualizer/` | Diff before replacing; Everywear currently carries a smaller port than upstream S3. |
| `lib/lrcParser.ts` | Shared lyrics utility | `packages/lyrics/` or `packages/shared/src/lib/` | Hoist if Gener8 + Vid both continue to consume it. |
| `context/intentBus.ts`, `types.ts` | Cross-applet types/events | `packages/shared/` | Hoist as typed Everywear primitives, not applet-local copies. |
| `components/CreatePanel.tsx` | Gener8 create workflow | `applets/gener8/web/src/views/create/` | Split into form components, generation state hook, tier/model controls, and shim API adapter. |
| `components/studio/StemStudio.tsx` | Studio Pro stem workflow | `applets/gener8/web/src/views/stems/` or future `packages/studio-workflows/` | Split before porting; keep backend calls behind Gener8 shim transport. |
| `components/studio/CompletePanel.tsx` | Studio Pro completion workflow | `applets/gener8/web/src/views/studio/complete/` | Port as a view module with isolated state and API adapter. |
| `services/api.ts` | Online S3 API client | `applets/gener8/web/src/lib/shimApi.ts` plus shell transport wrappers | Do not copy wholesale; separate Supabase-only online calls from Everywear-local shim calls. |
| `tour/catalog/*` | In-app tours | `packages/tour/` if reused, otherwise applet-local `views/*/tour.ts` | Keep each tour catalog small and colocated with the view it explains. |
...
### 3.1 Create Gener8 Applet (Context-Bounded Port)

**Objective:** Single Gener8 applet package with all creative capabilities, but
not a monolithic code shape. ACE-Step remains bundled with the applet. The tier
reconciler remains per-applet. The dependency bootstrap remains per-applet until
Zendit. Event-driven job communication remains the runtime spine.

**Context rule:** the tree below is a logical ownership map, not permission to
create large files. Any file or module group that approaches the 16k-token code
slot must follow the split plan in `ARCHITECTURE_MODULES_2026-05-21.md`.
...
**Objective:** Bring current S3 Studio / Gener8 web behaviour into the
S3-derived Everywear applets, starting with Gener8, while preserving Everywear's
shell-owned chrome, local shim transport, and context-bounded module layout.

**Non-negotiables:**
- do not copy oversized S3 web files directly;
- do not reintroduce Supabase-first assumptions into Everywear applet views;
- preserve S3 behaviour by moving features into smaller Everywear views,
  hooks, packages, and transport adapters;
- update module wiki entries as each surface lands.

**Port order:**

1. **Shared video surface first.** Split S3 / Gener8 / Vid
   `VideoGeneratorModal.tsx` into a `packages/video-modal/` package. Apply the
   latest S3 delta to that package once. Gener8 and Vid consume the same package
   with applet-specific props.
2. **Shared worker and visualizer.** Move `videoRenderWorker.ts` to
   `packages/video-modal/src/workers/`. Move visualizer primitives to
   `packages/visualizer/` after diffing S3's larger `silhouetteEngine.ts`
   against the current Everywear port.
3. **Create workflow.** Port `CreatePanel.tsx` into
   `applets/gener8/web/src/views/create/` as separate form, model/tier controls,
   generation-state hook, and shim API adapter.
4. **Studio workflows.** Port `StemStudio`, `CompletePanel`, Better Models, and
   related Studio Pro panels as independent view modules. Each module owns one
   workflow and one transport adapter.
5. **Tours.** Port tour catalogs only after their target views exist. Shared
   tour primitives can become `packages/tour/`; per-view copy stays colocated.

**Frontend acceptance criteria:**
- `npm run build --workspace applets/gener8/web` passes or fails only on
  pre-existing known errors explicitly documented in `CONTEXT.md`;
- `npm run build --workspace applets/vid/web` passes after the shared video
  package swap;
- no migrated TS/TSX source file exceeds the 16k-token code ceiling;
- every shared import comes from a workspace package, not from another applet's
  source tree;
- the S3 web source path and date of the migrated delta are recorded in the
  module wiki entry.
...
| 16 | S3 web deltas land through split modules/packages only | Any migrated TS/TSX file exceeds 16k tokens |
| 17 | Shared video surface consumed by Gener8 and Vid from one package | Duplicate modal or worker remains applet-local |
| 18 | Module Contract wiki page updated for each migrated web surface | Wiki/source drift |
...
| S3 web monolith copied into Everywear | High without discipline | High | Split by workflow first; hoist shared surfaces to packages; enforce 16k-token source ceiling. |
| Gener8 and Vid video tools drift | High | Medium | `packages/video-modal/` owns modal, worker, render types, and shared tests. |
| Online S3 API assumptions leak into applets | Medium | Medium | Port `services/api.ts` by concern; local features call Gener8 shim or shell transport. |
