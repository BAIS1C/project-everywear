...
import { useSongStore, readHasSongsHint } from './SongStoreContext';

/**
 * S³ Vid — Standalone applet wrapping the VideoGeneratorModal.
 * Promoted from modal to full window. Handoff-first: Gener8 sends a track
 * through intentBus, standalone entry opens a Vault picker on demand.
 *
 * 2026-05-02 SGT real-refactor: VidApp now consumes useSongStore()
 * directly instead of receiving songs via props. Empty-state branch
 * distinguishes between "loading your library" (returning user mid-fetch)
 * and "no songs yet" (fresh user, fetch settled, list genuinely empty).
 */

interface VidAppProps {
  /** Optional: kept on the public type for backward compat with callers
   *  that still pass songs/onPlaySong; both are ignored when present
   *  because the store is the source of truth. */
  songs?: Song[];
  onPlaySong?: (song: Song) => void;
}

export default function VidApp(_props: VidAppProps = {}) {
  ...
  return (
    <div className="flex h-full bg-s3 text-white" data-tour="vid.applet-root">
      {/* ── Main Content ──────────────────────────────────────── */}
      <div className="flex-1 flex flex-col min-w-0">
        {/* Tab bar */}
        <div className="flex items-center justify-between border-b border-white/[0.06] px-4 shrink-0">
          <div className="flex">
            <button
              onClick={() => setActiveTab('visualiser')}
              data-tour="vid.subtab.visualiser"
        ...
        <div className="flex-1 overflow-auto">
          {activeTab === 'visualiser' && (
            selectedSong ? (
              // The VideoGeneratorModal content rendered inline (not as a modal)
              <VideoGeneratorModal
                isOpen={true}
                onClose={() => {}} // No-op — we're not a modal
                song={selectedSong}
                embedded={true}
              />
            ) : (
              <div className="flex items-center justify-center h-full">
                <div className="text-center">
                  ...
                  <p className="text-sm text-[color:var(--ew-text-muted)] font-medium">Open a track to create a video</p>
                  <button
                    type="button"
                    onClick={() => setPickerOpen(true)}
            ...
            <div className="flex items-center justify-center h-full px-8">
              <div className="max-w-md text-center">
                ...
                <p className="text-sm text-white/40 leading-relaxed mb-4">
                  Coming with S³ Vid Pro. Generate beat-synced AI video from your tracks using
                ...
                <p className="text-xs text-accent-400/60">
                  The Visualiser tab is fully functional now — try it while AI Video is in development.
                </p>
              </div>
            </div>
          )}
          ...
          {activeTab === 'storyboard' && (
            <div className="flex items-center justify-center h-full px-8">
              <div className="max-w-md text-center">
                ...
                <p className="text-sm text-white/40 leading-relaxed mb-4">
                  Coming with S³ Vid Pro. Plan your music video scene-by-scene with AI-assisted
                ...
                <div className="space-y-2 text-left mb-6">
                  {[
                    ...
                    'Full S³ Vid visualiser integration',
                  ].map((f, i) => (
                    <div key={i} className="flex items-start gap-2">
