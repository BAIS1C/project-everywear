...
import { Play, Pause, SkipBack, SkipForward, Repeat, Shuffle, FolderOpen, Heart, MoreVertical, Volume2, VolumeX, Maximize2, Repeat1, ChevronDown, ChevronUp } from 'lucide-react';
...
import { analyseWaveformCached } from './studio/waveformAnalyser';
...
export const Player: React.FC<PlayerProps> = ({
    ...
    // Task #60: real waveform peaks for the bottom transport strip.
    // analyseWaveformCached wraps a memory + localStorage cache, so
    // repeated plays of the same track don't re-decode.
    const [peaks, setPeaks] = useState<number[]>([]);
    useEffect(() => {
        let cancelled = false;
        if (!currentSong?.audioUrl) { setPeaks([]); return; }
        analyseWaveformCached(currentSong.audioUrl, 400)
    ...
    /**
     * Open the locally-generated track in the OS file explorer.
     * Renamed from handleDownload 2026-05-01 SGT — tracks live on disk
     * so the right primitive is a folder reveal, not a fetch+blob
     * download. Posts to /api/launcher/reveal-in-folder; shim spawns
     * explorer.exe /select,<path> on Windows. Route is added in
     * PHASE F1; until then this 404s gracefully.
     */
    const handleOpenLocation = async () => {
        const audioUrl = currentSong?.audioUrl || (currentSong as any)?.audio_url;
        if (!audioUrl) return;
        try {
    ...
    return (
        <div className="h-20 lg:h-24 bg-white dark:bg-black/95 backdrop-blur border-t border-zinc-200 dark:border-white/10 flex flex-col z-50 transition-colors duration-300 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] dark:shadow-none">
            ...
            <div
                ref={progressBarRef}
                className="w-full cursor-pointer group relative bg-zinc-50 dark:bg-black/40"
                onClick={(e) => handleSeekInteraction(e, progressBarRef)}
            ...
            >
                {peaks.length > 0 ? (
                    ...
                    // player never renders empty during the ~200-600 ms
                    // between audioUrl change and first paint.
                    <div className="h-1 lg:h-1.5 bg-zinc-200 dark:bg-zinc-800 relative">
                        <div
                ...
                <div className="absolute top-1/2 -translate-y-1/2 w-full h-10 -z-10"></div>
            </div>
