...
export const EditProfileModal: React.FC<EditProfileModalProps> = ({ isOpen, onClose, onSaved }) => {
    ...
    const loadProfile = async () => {
        ...
        setLoading(true);
        try {
            const rawName = user.raw_username || user.username.replace(/@everywear\.id$/, '');
            const res = await usersApi.getProfile(rawName, token);
            setProfile(res.user);
            setEditUsername(res.user.raw_username || res.user.username.replace(/@everywear\.id$/, '') || '');
    ...
    const handleSaveProfile = async () => {
        ...
        try {
            // Username mutation DISABLED: Everywear IDs are immutable (future wallet address).
            // editUsername field is read-only in the UI; skip any update attempt.
                ...
                {loading ? (
                    ...
                    <>
                        <div className="p-6 space-y-6">
                            {/* Username (IMMUTABLE: Everywear ID = future wallet address) */}
                            <div className="space-y-2">
                                <label className="text-sm font-medium text-zinc-300">Everywear ID</label>
                                <div className="flex items-center gap-2">
                                    <input
                                        type="text"
                                        value={editUsername ? `${editUsername}@everywear.id` : ''}
                                        readOnly
                                        disabled
                                        className="flex-1 bg-zinc-900/50 border border-zinc-800 rounded-lg px-3 py-2 text-zinc-400 cursor-not-allowed"
                                    />
                                </div>
                                <p className="text-xs text-zinc-600">Your Everywear ID is permanent and cannot be changed.</p>
