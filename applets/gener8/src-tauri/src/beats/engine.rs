...
//!
//! symphonia handles the decode side (WAV / MP3 / FLAC / OGG / AAC), aubio
//! handles the beat tracking. Intentionally single-threaded: beat analysis
//! on a 4-minute track takes under a second on modern CPUs, and serialising
//! at the handler level avoids aubio-internal thread-safety surprises.
//!
...
/// Analysis output. Kept flat for straightforward JSON serialisation.
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct BeatMap {
    /// Estimated BPM. Continuous; aubio outputs a floating-point tempo.
    pub bpm: f32,
    /// Total audio duration in milliseconds.
    pub duration_ms: u64,
    /// Sample rate the analysis ran at.
    pub sample_rate: u32,
    /// Beat positions in milliseconds, monotonic, sorted.
    pub beats: Vec<u64>,
    /// Downbeat markers in milliseconds (every 4 beats as a v1 heuristic).
    pub downbeats: Vec<u64>,
    /// Section boundaries in milliseconds (intro / body / outro etc). V1:
    /// equal quartile split; v2 will derive from onset-density shifts.
    pub sections: Vec<u64>,
    /// "aubio-default" for now; reserved for alternative backends.
...
/// tracking. `target_sr` is advisory; if the source sample rate differs we
/// analyse at the source rate rather than introduce a resampler dependency.
...
/// Decode any symphonia-supported container -> mono f32 + sample rate.
...
/// Naive linear resample. Adequate for beat tracking; do not use for audio
...
/// Run aubio tempo tracking. Returns beat positions in milliseconds.
...
/// BPM from beat intervals: median reciprocal x 60,000.
...
/// V1 sectioning: equal quartile split.
