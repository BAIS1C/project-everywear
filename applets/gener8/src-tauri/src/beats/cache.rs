...
//!
//! Key shape: `{sha256(abs_path)[0..16]}_mt{mtime_ms}_sr{sample_rate}.json`
//!
//! Disk location: everywear_paths::data_dir("gener8")/cache/beats/
//!
//! Ported from S3 Studio. Key migration changes:
//!   - Cache dir resolved by caller via everywear_paths (not util::app_data_dir)
...
/// Wire-format record persisted on disk.
...
impl BeatsCache {
    ...
    /// Attempt a lookup. Returns `Some(map)` if both the key and the mtime
    /// still match the on-disk audio file.
    ...
    /// Persist the analysis. Failure to write is non-fatal.
    ...
    /// Oldest-first eviction until disk usage sits at or below the quota.
...
mod tests {
    ...
    fn tmpdir() -> PathBuf {
        ...
        p.push(format!("gener8-beats-test-{}", std::process::id()));
    ...
    fn mtime_invalidates() {
        ...
        assert!(
            cache.get(&audio, 44_100).is_none(),
    ...
    fn sample_rate_partitions_cache() {
        ...
        assert!(cache.get(&audio, 44_100).is_some());
        assert!(
            cache.get(&audio, 22_050).is_none(),
            "different sample rate must miss"
        );
    }
}
