...
import { downloadGLB, downloadVRMWithAvatar, exportToStrands } from "../library/download-utils"
...
export class CharacterManager {
  ...
  screenshotManager
    ...
    /**
     * Exports VRM + Strands Avatar Manifest to a user-chosen directory.
     * Used for Kasai Local companion and Strands Nation avatar pipeline.
     */
    async exportToStrands(name, exportOptions = null) {
      if (!this.canDownload()) {
        throw new Error("Export not supported: no character loaded.")
      }
      exportOptions = exportOptions || {}
      const manifestOptions = this.manifestDataManager.getExportOptions()
      const finalOptions = { ...manifestOptions, ...exportOptions }
      ...
      // Collect trait summary for the sidecar manifest
      const traitSummary = this.getAvatarSelection()

      return exportToStrands(
        this.characterModel,
        this.avatar,
        name,
        finalOptions,
    ...
    loadInitialTraits() {
      return new Promise(async(resolve, reject) => {
        ...
        if (this.manifestDataManager.hasExistingManifest()) {
          ...
          // Play a looping standing idle so the avatar is not stuck in bind/T-pose
          // after load. This is the true single-fire point: trait swaps use
          // loadTrait / loadRandomTraits and do not re-trigger it. Wrapped so a
          // missing/failed idle never blocks character loading.
          try {
            if (this.animationManager) {
              await this.animationManager.loadAnimation(
                `${ASSET_BASE}/lora-assets/animations/FemaleStandingPose.fbx`,
                false, 0, true, ""
              );
            }
          } catch (err) {
            console.warn("Idle animation failed to load:", err);
          }
