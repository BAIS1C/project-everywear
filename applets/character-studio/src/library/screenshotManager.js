 ...
 */
  ...
  _createImage(width, height, pixelSize = null){
    ...
    // Hide the editor-only beauty rig (rim/fill lights, backdrop, contact shadow)
    // so exported pixels stay identical to the pre-rig deterministic output.
    const editorRig = this.scene.getObjectByName("STRANDS_EDITOR_RIG");
    const rigWasVisible = editorRig ? editorRig.visible : false;
    if (editorRig) editorRig.visible = false;
    ...
    try {
      ...
      if (editorRig) editorRig.visible = rigWasVisible;
      return  imgData
    } catch (e) {
      console.error(e);
      if (editorRig) editorRig.visible = rigWasVisible;
