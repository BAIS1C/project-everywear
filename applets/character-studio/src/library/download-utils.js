...
/**
 * Returns the optimized VRM as a raw ArrayBuffer without triggering download.
 * Used by the Strands export pipeline (Kasai Local, Strands Nation).
 */
export async function getExportableVRM(model, avatar, fileName, options) {
  const vrmData = await getVRMData(model, avatar, options)
  return {
    fileName: (fileName || "AvatarCreatorModel") + ".vrm",
    buffer: vrmData
  }
}

/**
 * Exports VRM + sidecar manifest using File System Access API.
 * Falls back to regular browser download if API unavailable.
 */
export async function exportToStrands(model, avatar, fileName, options, traitSummary = {}) {
  const { buffer } = await getExportableVRM(model, avatar, fileName, options)
  const safeName = (fileName || "kasai-avatar").replace(/[^a-zA-Z0-9_-]/g, "_")

  // Build Strands Avatar Manifest (sidecar JSON)
  const manifest = {
  ...
  if (window.showDirectoryPicker) {
    try {
      const dirHandle = await window.showDirectoryPicker({
        id: "strands-avatar-export",
        mode: "readwrite",
        startIn: "desktop",
      })

      // Write VRM file
      const vrmHandle = await dirHandle.getFileHandle(`${safeName}.vrm`, { create: true })
      const vrmWritable = await vrmHandle.createWritable()
      await vrmWritable.write(buffer)
      await vrmWritable.close()

      // Write sidecar manifest
      const manifestHandle = await dirHandle.getFileHandle("strands-avatar.json", { create: true })
      const manifestWritable = await manifestHandle.createWritable()
      await manifestWritable.write(JSON.stringify(manifest, null, 2))
      await manifestWritable.close()

      return { success: true, method: "filesystem", manifest }
    } catch (err) {
  ...
  saveArrayBuffer(buffer, `${safeName}.vrm`)
